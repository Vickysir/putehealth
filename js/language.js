...
function translateIndexPages(data) {
  if (data) {
    ...
    // 4个产品
    for (let i = 0; i < 4; i++) {
      $('#product .media-image-body' + (i + 1) + ' h2').text(
        data.products['product-' + (i + 1)].title
      );
      $('#product .media-image-body' + (i + 1) + ' p').text(
        data.products['product-' + (i + 1)].content
      );
    }
    ...
    $('#knowledge .headlines-content-summary .qhd-content p').text(
      data.news.latast.infor
    );
    // 3个新闻
    for (let i = 0; i < 3; i++) {
    ...
    for (let i = 0; i < 6; i++) {
      $('#contact .form' + (i + 1) + ' .contact-label').html(
        data.connect.form[i][0]
      );
      $('#contact .form' + (i + 1) + ' .form-control').attr(
        'placeholder',
        data.connect.form[i][1]
      );
    }
    // subumit
